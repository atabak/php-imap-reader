...
use Exception;
use IMAP\Connection;
use Webicdesign\Dump;
use Webicdesign\File_Manager;
...
class Reader
{
    ...
    public function getEmail(int $uid): ?Email
    {
        ...
        $header_from_body = imap_fetchbody($this->stream(), $uid, '0', FT_PEEK);
        //new Dump($header_from_body);die();
        ...
        $body = imap_fetchstructure($this->stream(), $uid);
        //new Dump($body);
     ...
     * @throws Exception
     */
    public function viewEmail($email_number): array
    {
        $data['overview'] = imap_fetch_overview($this->stream(), $email_number, 0);
        $data['body']     = imap_fetchbody($this->stream(), $email_number, "1.2");
        /* get mail structure */
        ...
        if (isset($structure->parts) && count($structure->parts)) {
            for ($i = 0; $i < count($structure->parts); $i++) {
                ...
                if ($attachments[$i]['is_attachment']) {
                    $attachments[$i]['attachment'] = imap_fetchbody($this->stream(), $email_number, $i + 1);
                    /* 3 = BASE64 encoding */
        ...
        foreach ($attachments as $attachment) {
            if ($attachment['is_attachment'] == 1) {
                ...
                $fp = fopen(File_Manager::TMP_DIR . $email_number . "-" . $filename, "w+");
                fwrite($fp, $attachment['attachment']);
                fclose($fp);
                $data['attachments'][] = [
                    'file' => File_Manager::TMP_DIR . $email_number . "-" . $filename,
